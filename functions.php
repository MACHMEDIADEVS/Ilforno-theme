...
function il_forno_a_legna_setup() {
	/*
		...
		* If you're building a theme based on IL Forno a Legna, use a find and replace
		* to change 'il-forno-a-legna' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'il-forno-a-legna' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'il_forno_a_legna_custom_background_args',
			array(
...
function il_forno_a_legna_scripts() {
	wp_enqueue_style( 'il-forno-a-legna-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'il-forno-a-legna-style', 'rtl', 'replace' );

	wp_enqueue_script( 'il-forno-a-legna-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
